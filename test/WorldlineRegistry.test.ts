...
describe("WorldlineRegistry", function () {
  async function deployFixture() {
    ...
    const registry: WorldlineRegistry = await Registry.deploy(
  ...
  describe("deployment", function () {
    ...
    it("stores the default verifier address", async function () {
      ...
      expect(await registry.defaultVerifier()).to.equal(
        await verifier.getAddress()
      );
    });
  ...
  describe("registerCircuit", function () {
    ...
    it("reverts on duplicate circuit ID", async function () {
      ...
      await registry
        .connect(owner)
        .registerCircuit(CIRCUIT_ID, "first", ethers.ZeroAddress, "");
      await expect(
        registry
          .connect(owner)
          .registerCircuit(CIRCUIT_ID, "second", ethers.ZeroAddress, "")
    ...
    it("reverts when circuit ID is zero bytes32", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerCircuit(ZERO_BYTES32, "desc", ethers.ZeroAddress, "")
    ...
    it("registers with address(0) verifier (falls back to defaultVerifier)", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerCircuit(CIRCUIT_ID, "desc", ethers.ZeroAddress, "uri")
      ).to.not.be.reverted;
    });
  });
  ...
  describe("getCircuit", function () {
    ...
    it("reverts for an unknown circuit ID", async function () {
      ...
      await expect(registry.getCircuit(CIRCUIT_ID)).to.be.revertedWith(
        "circuit missing"
      );
    });
  });
  ...
  describe("registerDriver", function () {
    it("admin can register a driver", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerDriver(DRIVER_ID, "1.0.0", "http://localhost:8545")
      )
    ...
    it("reverts on duplicate driver ID", async function () {
      ...
      await registry
        .connect(owner)
        .registerDriver(DRIVER_ID, "1.0.0", "http://a");
      await expect(
        registry
          .connect(owner)
          .registerDriver(DRIVER_ID, "2.0.0", "http://b")
    ...
    it("reverts when driver ID is zero bytes32", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerDriver(ZERO_BYTES32, "1.0.0", "http://x")
  ...
  describe("getDriver", function () {
    it("returns correct metadata for a registered driver", async function () {
      ...
      await registry
        .connect(owner)
        .registerDriver(DRIVER_ID, "1.2.3", "http://rpc.example.com");
    ...
    it("reverts for an unknown driver ID", async function () {
      const { registry } = await loadFixture(deployFixture);
      await expect(registry.getDriver(DRIVER_ID)).to.be.revertedWith(
        "driver missing"
      );
    });
  });
  ...
  describe("registerPlugin", function () {
    ...
    it("reverts on duplicate plugin ID", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerPlugin(PLUGIN_ID, "2.0.0", stranger.address, CIRCUIT_ID)
    ...
    it("reverts when plugin ID is zero bytes32", async function () {
      ...
      await expect(
        registry
          .connect(owner)
          .registerPlugin(ZERO_BYTES32, "1.0.0", stranger.address, CIRCUIT_ID)
  ...
  describe("deprecatePlugin", function () {
    async function withPlugin(
      ...
      await registry
        .connect(owner)
        .registerCircuit(CIRCUIT_ID, "c", ethers.ZeroAddress, "");
      await registry
        .connect(owner)
        .registerPlugin(PLUGIN_ID, "1.0.0", implAddr, CIRCUIT_ID);
    }
    ...
    it("reverts when deprecating a non-existent plugin", async function () {
      ...
      ).to.be.revertedWith("plugin missing");
    });
  });
  ...
  describe("getPlugin", function () {
    ...
    it("reverts for an unknown plugin ID", async function () {
      const { registry } = await loadFixture(deployFixture);
      await expect(registry.getPlugin(PLUGIN_ID)).to.be.revertedWith(
        "plugin missing"
      );
    });
  });
  ...
  describe("verify", function () {
    it("returns true for a valid proof using the defaultVerifier (circuit verifier=0)", async function () {
      ...
      await registry
        .connect(owner)
        .registerCircuit(CIRCUIT_ID, "sq", ethers.ZeroAddress, "");
      // 3² = 9
    ...
    it("returns true for a valid proof using a circuit-specific verifier", async function () {
      ...
      await registry
        .connect(owner)
        .registerCircuit(CIRCUIT_ID, "sq", verifierAddr, "");
    ...
    it("reverts when circuit ID is not registered", async function () {
      const { registry } = await loadFixture(deployFixture);
      await expect(
        registry.verify(CIRCUIT_ID, 3n, 9n)
      ).to.be.revertedWith("circuit missing");
    });
  });
});
