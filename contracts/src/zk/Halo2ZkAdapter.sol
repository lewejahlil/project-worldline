...
///
///      proof = abi.encode(
///          bytes   proofBytes,         // Raw Halo2 KZG proof (variable length, ~2016 bytes)
...
contract Halo2ZkAdapter is IZkAggregatorVerifier, IZkAdapter {
  ...
  /// @dev Raw proof (~2016 bytes) + 2 × 32 bytes for stfCommitment and proverSetDigest.
  ///      ABI encoding overhead: offset(32) + stf(32) + psd(32) + length(32) + proof(2016) = 2144.
  ///      We use a conservative minimum that accounts for ABI encoding overhead.
  uint256 public constant HALO2_PROOF_MIN_LEN = 2144;
  ...
  function verify(
    ...
    (bytes memory proofBytes, uint256 stfCommitmentUint, uint256 proverSetDigestUint) = abi.decode(
      proof,
